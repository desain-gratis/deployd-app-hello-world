...
func (h *handler) MessageOfTheDay(w http.ResponseWriter, r *http.Request, p httprouter.Params) {
	w.WriteHeader(http.StatusOK)

	motd := os.Getenv("MOTD")
	if motd == "" {
		motd = "MOTD env is empty. Please configure it in your Deployd cluster :)"
	}
	fmt.Fprintf(w, "%s\n", motd)
}
